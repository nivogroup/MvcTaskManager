...
namespace MvcTaskManager.Services
{
    public class UsersService : IUsersService
    {
        ...
        public async Task<ApplicationUser> Register(SignUpViewModel signUpViewModel)
        {
            ...
            if (result.Succeeded)
            {
                if ((await _applicationUserManager.AddToRoleAsync(await _applicationUserManager.FindByNameAsync(signUpViewModel.Email), 
                    "Employee")).Succeeded)
                {
                    ...
                    if (result2.Succeeded)
                    {
                        ...
                        new Claim(ClaimTypes.Role, applicationUser.Role)
                            ...
                            Expires = DateTime.UtcNow.AddHours(8),
                            SigningCredentials = new Microsoft.IdentityModel.Tokens.SigningCredentials(new 
